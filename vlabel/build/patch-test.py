...
data_file = "/home/ubuntu/zxy/SubgraphMatchingSurvey/vlabel/test/data_graph/HPRD.graph"  # 数据文件路径
query_files_list = "/home/ubuntu/zxy/SubgraphMatchingSurvey/vlabel/test/data_graph/sub200/enum.txt"  # 包含所有查询数据集的文件路径
...
# 创建日志目录
logdir = "hrpd_main_200_timeout"
os.makedirs(logdir, exist_ok=True)
# for logdir in logdirs:
#     os.makedirs(logdir, exist_ok=True)

# 读取查询数据集文件
base_directory = "/home/ubuntu/zxy/SubgraphMatchingSurvey/vlabel/test/data_graph/sub200"
    ...
    for filter_param, order_param, engine_param, logdir_param in zip(filters, orders, engines, logdirs):
        ...
        with open(output_file, 'a') as f:
            ...
            for query_file in files:
                ...
                f.write("=" * 50 + "\n")
                try:
                    subprocess.run(command, stdout=f, stderr=subprocess.STDOUT, timeout=timeout_duration)
                except subprocess.TimeoutExpired:
                    f.write(f"Command timed out after {timeout_duration} seconds.\n")
                f.write("\n\n")  # 添加分隔符
